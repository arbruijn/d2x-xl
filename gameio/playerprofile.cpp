...
if (!m_cf.Open (fn, gameFolders.szProfiles, "wt", 0))
	return 0;
...
sprintf (fn, "%s.plx", LOCALPLAYER.callsign);
if (!m_cf.Open (fn, gameFolders.szProfiles, "rb", 0))
	return 0;
while (!m_cf.EoF ()) {
	LoadParam ();
...
tParamValue defaultParams [] = {
	 ...
	 {"extraGameInfo[0].shipsAllowed[2]", "1"},
...
if (!cf.Open (filename, gameFolders.szProfiles, "rb", 0)) {
...
gameOpts->render.effects.bGlow = ::Clamp (gameOpts->render.effects.bGlow, 0, 1);
...
cf.Open (filename, gameFolders.szProfiles, "wb", 0);
if (cf.File ()) {
...
for (;;) {
	...
	if (cf.Exist (filename,gameFolders.szProfiles, 0)) {
...
bStartup = 0;
sprintf (filespec, "%s%s*.plr", gameFolders.szProfiles, *gameFolders.szProfiles ? "/" : "");
