 ...
 */
static uint16_t baseport = UDP_BASEPORT;
...
va_end (argP);   
TextBox (NULL, BG_STANDARD, 1, "OK", "UDP Error\n\n%s\nError code: %d", szFailMsg, WSAGetLastError ());
//Warning ("%s\nError code: %d", szFailMsg, WSAGetLastError ());
return 1;
}
 ...
 */
static void dumpraddr (uint8_t *a)
{
	int16_t port;

	//printf ("[%u.%u.%u.%u]", a[0], a[1], a[2], a[3]);
	port= (signed int16_t)ntohs (*reinterpret_cast<uint16_t*> (a+4);
...
// game_addr = inet_makeaddr (239*256 + 255, RandShort () % 0xFFFF);
sprintf (addr, "%i.%i.%i.%i", 239, 255, RandShort () % 0xFF, RandShort () % 0xFF);
...
extern int32_t UDPGetMyAddress (void);
