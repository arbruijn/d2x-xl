...
int32_t CDynLight::SeesPoint (const int16_t nDestSeg, const int8_t nDestSide, const CFixVector* pvNormal, CFixVector* pvPoint, int32_t nLevel, int32_t nThread)
{
ENTER (0, 0);
...
if (info.nSide < 0) {
	...
	if (CFloatVector::Dot (vLightToPointf, info.vDirf) < -0.001f) // light doesn't see point
		RETVAL (0)
	if (pvNormal) {
		vNormalf.Assign (*pvNormal);
		if (CFloatVector::Dot (vLightToPointf, vNormalf) > 0.001f) // light doesn't "see" face
			RETVAL (0)
		}
	RETVAL ((nDestSeg < 0) || PointSeesPoint (&v0, &v1, info.nSegment, nDestSeg, nDestSide, 0, nThread));
	}
else {
		class CLightPoint {
			public:
	...
	for (i = 0; i < nVertices; i++) {
		...
		if (pvNormal && (CFloatVector::Dot (vLightToPointf, vNormalf) > 0.001f)) // light doesn't "see" face
			continue;
#else
		int32_t bLightSeesFace = (CFloatVector::Dot (vLightToPointf, info.vDirf) > -0.001f) && (!pvNormal || (CFloatVector::Dot (vLightToPointf, vNormalf) < 0.001f));
#endif
	...
	RETVAL (0) 
#if DBG
	if ((nDbgSeg >= 0) && (nDbgVertex >= 0) && (info.nSegment == nDbgSeg) && ((nDbgSide < 0) || (info.nSide == nDbgSide)) && (nDbgVertex >= 0) && (*pvPoint == VERTICES [nDbgVertex]))
		BRP;
#endif
	}
...
CHitQuery hitQuery (FQ_TRANSWALL | FQ_TRANSPOINT | FQ_VISIBILITY, &info.vPos, pvPoint, info.nSegment, -1, 1, 0);
...
if (nLightSeg == nDestSeg)
	...
	else if (!gameData.segData.SegVis (nLightSeg, nDestSeg))
...
if (info.bDiffuse [nThread] == 2) 
	info.bDiffuse [nThread] = 1;
else if (info.bDiffuse [nThread] == -1) 
	info.bDiffuse [nThread] = 0;
else { // => ambient contribution only
