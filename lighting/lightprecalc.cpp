...
if (nSegment == nDbgSeg)
	nDbgSeg = nDbgSeg;
#endif
dacsRouter [nThread].PathLength (CFixVector::ZERO, nSegment, CFixVector::ZERO, -1, 0x7FFFFFFF, WID_SEETHRU_FLAG | WID_PASSABLE_FLAG, -1);
		...
		if (IsLightVert (nVertex, lightP->info.faceP))
			...
			if ((lightP->info.nSegment >= 0) && (lightP->info.nSide >= 0)) {
				...
				if ((CFixVector::Dot (sideP->m_normals [0], vLightToVert) < 0) &&
					...
					h = simpleRouter [nThread].PathLength (VERTICES [nVertex], -1, lightP->info.vPos, lightP->info.nSegment, X2I (xMaxLightRange / 5), WID_SEETHRU_FLAG | WID_PASSABLE_FLAG, 0);
