...
if (gameStates.render.cameras.bActive) {
	...
	if ((gameData.objs.viewerP == gameData.objs.consoleP) && transformation.m_info.bUsePlayerHeadAngles) {
		...
		gameStates.render.nMinZoomFactor = I2X (gameStates.render.glAspect); //(((cockpit->Mode () == CM_FULL_COCKPIT) ? I2X (2)  / 3 : I2X (1)) * glAspect);
		...
		if ((gameData.weapons.nPrimary != VULCAN_INDEX) && (gameData.weapons.nPrimary != GAUSS_INDEX))
			gameStates.render.nZoomFactor = gameStates.render.nMinZoomFactor; //(fix) (((cockpit->Mode () == CM_FULL_COCKPIT) ? I2X (2)  / 3 : I2X (1)) * glAspect);
		else {
			switch (extraGameInfo [IsMultiGame].nZoomMode) {
				...
				case 1:
					if (ZoomKeyPressed ()) {
						if (!bStopZoom) {
							gameStates.render.nZoomFactor = (cockpit->Mode () == CM_FULL_COCKPIT) ? (gameStates.render.nZoomFactor * 7) / 5 : (gameStates.render.nZoomFactor * 5) / 3;
