...
if (gameOpts->render.textures.bUseHires [0] || gameOpts->render.effects.bTransparent) {
	if (fAlpha) {
		bAdditive = ((nType == OBJ_FIREBALL) || (nType == OBJ_EXPLOSION)) ? /*2*/1 : ((nType == OBJ_WEAPON) && (objP->info.nId == OMEGA_ID)) ? 2 : 0;
#if 0
...
switch (objP->info.renderType) {
	...
	case RT_POLYOBJ:
		if (nType == OBJ_EFFECT) {
			objP->info.renderType = (objP->info.nId == SMOKE_ID) ? RT_SMOKE : (objP->info.nId == LIGHTNING_ID) ? RT_LIGHTNING : RT_SOUND;
			return 0;
			}
		if (nType == OBJ_PLAYER) {
