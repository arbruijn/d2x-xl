	...
	for (i = 0, pointPP = pointList; i < nVertices; i++, pointPP++) {
		...
		if (bDynLight)
			G3VertexColor (-1, -1, vertIndex [i], pointP->GetNormal ()->XYZ (), vVertPos.XYZ (), vertColors + i, NULL,
								gameStates.render.nState ? X2F (uvlList [i].l) : 1, -1, 0, 0);
		else if (bLight)
...
if (bDynLight) {
	if (bShaderMerge) {
		for (i = 0, pointPP = pointList; i < nVertices; i++, pointPP++) {
			pointP = *pointPP;
			G3VERTPOS (vVertPos, pointP);
			G3VertexColor (-1, -1, pointP->Index (), pointP->GetNormal ()->XYZ (), vVertPos.XYZ (), &faceColor, NULL, 1, 0, 0, 0);
		...
		for (i = 0, pointPP = pointList; i < nVertices; i++, pointPP++) {
			...
			G3VertexColor (-1, -1, pointP->Index (), pointP->GetNormal ()->XYZ (), vVertPos.XYZ (), &faceColor, NULL,
								gameStates.render.nState ? X2F (uvlList [i].l) : 1, 0, 0, 0);
			ogl.ColorBuffer () [i].Assign (faceColor);
			ogl.TexCoordBuffer () [i].v.u = X2F (uvlList [i].u);
			ogl.TexCoordBuffer () [i].v.v = X2F (uvlList [i].v);
			ogl.VertexBuffer () [i] = vVertPos;
...
if (ogl.SizeBuffers (nVertices)) {
	if (bDynLight) {
		...
		for (i = 0, pointPP = pointList; i < nVertices; i++, pointPP++) {
			pointP = *pointPP;
			G3VERTPOS (vVertPos, pointP);
			G3VertexColor (-1, -1, pointP->Index (), pointP->GetNormal ()->XYZ (), vVertPos.XYZ (), NULL, &faceColor, 1, 0, 0, 0);
			ogl.ColorBuffer () [i].Assign (faceColor);
			ogl.TexCoordBuffer () [i].v.u = X2F (uvlList [i].u);
			ogl.TexCoordBuffer () [i].v.v = X2F (uvlList [i].v);
			ogl.VertexBuffer () [i] = vVertPos;
