...
#else
	return colorP ? (colorP->red * 3 + colorP->green * 5 + colorP->blue * 2)
			/ 10.0f : 1.0f;
#endif
}
...
int CParticle::Create(CFixVector *vPos, CFixVector *vDir, CFixMatrix *mOrient,
	...
	if (nScale < 0)
		...
		m_nRad
				= float(
						PARTICLE_SIZE (gameOpts->render.particles.nSize [0], nScale, m_bBlowUp));
	else
		m_nRad = float(nScale);
	if (!m_nRad)
		m_nRad = 1.0f;
	...
	m_nType = nType;
	m_bEmissive = (nParticleSystemType == LIGHT_PARTICLES) ? 1
			: (nParticleSystemType == FIRE_PARTICLES) ? 2
	...
	m_nSegment = nSegment;
	m_nBounce
			= ((m_nType == BUBBLE_PARTICLES) || (m_nType == RAIN_PARTICLES)
					|| (m_nType == SNOW_PARTICLES) || (m_nType
					== WATERFALL_PARTICLES)) ? 1 : 2;
	m_bReversed = 0;
	m_nMoved = nCurTime;
	if (nLife < 0)
		nLife = -nLife;
	...
	m_color[0] = m_color[1] = color = (colorP && (m_bEmissive != 2)) ? *colorP
			: defaultParticleColor;

	if ((nType == BULLET_PARTICLES) || (nType == BUBBLE_PARTICLES) || (m_nType
			== RAIN_PARTICLES) || (m_nType == SNOW_PARTICLES)) {
		m_bBright = 0;
		m_nFadeState = -1;
		...
		if (m_bEmissive /*&& (m_bEmissive < 3)*/)
			...
			if (!colorP)
				m_color[0].alpha = float(3 * SMOKE_START_ALPHA / 4 + randN(
						SMOKE_START_ALPHA / 2)) / 255.0f;
			else {
				if (colorP->alpha < 0) {
					ubyte a = ubyte(-colorP->alpha * 255.0f * 0.25f + 0.5f);
					m_color[0].alpha = float(3 * a + randN(2 * a)) / 255.0f;
	...
	else if ((nType != BUBBLE_PARTICLES) && (nType != RAIN_PARTICLES) && (nType
			!= SNOW_PARTICLES))
		m_vPos = *vPos + m_vDrift * (I2X (1) / 64);
	else {
		//m_vPos = *vPos + vDrift * (I2X (1) / 32);
		nSpeed = m_vDrift.Mag() / 16;
		CFixVector v = CFixVector::Avg((*mOrient).RVec() * (nSpeed - randN(2
				* nSpeed)), (*mOrient).UVec() * (nSpeed - randN(2 * nSpeed)));
	...
	m_bAnimate = (nType != FIRE_PARTICLES)
			&& (gameOpts->render.particles.nQuality > 1) && (m_nFrames > 1);
	m_bRotate = ((m_nRenderType <= SMOKE_PARTICLES) || (m_nRenderType
			== SNOW_PARTICLES)) ? 1 : (m_nRenderType == FIRE_PARTICLES
			+ PARTICLE_TYPES) ? -1 : 0;
...
#endif
	{
		if (m_bEmissive /*&& (m_bEmissive < 3)*/)
			...
			m_color[0].alpha /= 3.5f - float(
					gameOpts->render.particles.nQuality) / 2.0f; //colorP ? 2.0f + (color.red + color.green + color.blue) / 3.0f : 2.0f;
		else if (nParticleSystemType == SMOKE_PARTICLES)
			m_color[0].alpha /= colorP ? 3.0f - (color.red + color.green
					+ color.blue) / 3.0f : 2.5f;
		else if ((nParticleSystemType == BUBBLE_PARTICLES)
				|| (nParticleSystemType == RAIN_PARTICLES)
				|| (nParticleSystemType == SNOW_PARTICLES))
			m_color[0].alpha /= 2.0f;
		else if (nParticleSystemType == GATLING_PARTICLES)
...
bool CParticle::IsVisible(int nThread) {
	...
	if ((m_nSegment < 0) || (m_nSegment >= gameData.segs.nSegments))
		return false;
	if (gameData.render.mine.bVisible[m_nSegment]
			== gameData.render.mine.nVisible)
		return true;
	short* childP = SEGMENTS[m_nSegment].m_children;
	for (int i = 6; i; i--, childP++)
		if ((*childP >= 0) && (gameData.render.mine.bVisible[*childP]
				== gameData.render.mine.nVisible))
			return true;
	...
	if (nSegment < 0)
		return false;
	m_nSegment = nSegment;
	return gameData.render.mine.bVisible[nSegment]
			== gameData.render.mine.nVisible;
#endif
}
...
int CParticle::CollideWithWall(int nThread) {
	...
	for (nSide = 0, sideP = segP->m_sides; nSide < 6; nSide++, sideP++) {
		...
		for (nFace = nInFront = 0; nFace < nFaces; nFace++) {
			nDist = m_vPos.DistToPlane(sideP->m_normals[nFace],
					gameData.segs.vertices[sideP->m_nMinVertex[0]]);
			if (nDist > -PLANE_DIST_TOLERANCE)
				nInFront++;
			else
				wallNorm[nThread] = sideP->m_normals + nFace;
		}
		if (!nInFront || (bSidePokesOut[nThread][nSide] && (nFaces == 2)
				&& (nInFront < 2))) {
...
void CParticle::UpdateTexCoord(void) {
	if ((m_nType <= WATERFALL_PARTICLES) && ((m_nType != BUBBLE_PARTICLES)
			|| gameOpts->render.particles.bWobbleBubbles)) {
...
fix CParticle::Drag(void) {
	if ((m_nType == BUBBLE_PARTICLES) || (m_nType == RAIN_PARTICLES)
			|| (m_nType == SNOW_PARTICLES) || (m_nType == FIRE_PARTICLES))
...
int CParticle::Bounce(int nThread) {
	...
	if (!CollideWithWall(nThread)) {
		m_nBounce = ((m_nType == BUBBLE_PARTICLES) || (m_nType
				== RAIN_PARTICLES) || (m_nType == SNOW_PARTICLES) || (m_nType
				== WATERFALL_PARTICLES)) ? 1 : 2;
		return 1;
	}
	if (!--m_nBounce)
...
int CParticle::UpdateDrift(int t, int nThread) {
	...
	if (m_nSegment < -1)
		m_nSegment++;
	if (m_nSegment >= -1) {
		nSegment = FindSegByPos(m_vPos, m_nSegment, 1, -1, ((m_nType
				== BUBBLE_PARTICLES) || (m_nType == RAIN_PARTICLES) || (m_nType
				== SNOW_PARTICLES)) ? 0 : fix(m_nRad), nThread);
		if (nSegment < 0)
	...
	if ((nSegment < 0) && ((m_nType != WATERFALL_PARTICLES) || m_bChecked)) {
		if (m_nType == WATERFALL_PARTICLES) {
			...
			if ((CFixVector::Normalize(vDir) >= I2X (1)) && (CFixVector::Dot(
					vDir, m_vDir) < I2X (1) / 2)) {
				m_nLife = -1;
				return 0;
			}
			...
			&& !SEGMENTS[nSegment].HasWaterProp()) {
		m_nLife = -1;
		return 0;
	} else if ((m_nType == RAIN_PARTICLES) || (m_nType == SNOW_PARTICLES)) {
		if (SEGMENTS[nSegment].HasWaterProp()
				|| SEGMENTS[nSegment].HasLavaProp()) {
			m_nLife = -1;
			return 0;
		}
		if ((m_nSegment >= 0) && (nSegment != m_nSegment)
				&& SEGMENTS[m_nSegment].HasFunction(SEGMENT_FUNC_SKYBOX)) {
...
void CParticle::UpdateDecay(void) {
	if ((m_nType == BUBBLE_PARTICLES) || (m_nType == RAIN_PARTICLES)
			|| (m_nType == SNOW_PARTICLES) || (m_nType == WATERFALL_PARTICLES))
		m_decay = 1.0f;
...
int CParticle::RenderType(void) {
	...
	if ((m_nType != FIRE_PARTICLES)
			|| /*(gameOpts->render.particles.nQuality < 2) ||*/(m_iFrame
					< m_nFrames))
...
#else // -----------------------------------------------------------------------
void CParticle::Setup(float fBrightness, char nFrame, char nRotFrame,
		tParticleVertex* pb, int nThread) {
	...
	pb[m_nOrient].texCoord.v.u = pb[(m_nOrient + 3) % 4].texCoord.v.u
			= m_texCoord.v.u + hx;
	pb[(m_nOrient + 1) % 4].texCoord.v.u = pb[(m_nOrient + 2) % 4].texCoord.v.u
			= m_texCoord.v.u + m_deltaUV - hx;
	float hy = ParticleImageInfo(m_nType).yBorder;
	pb[m_nOrient].texCoord.v.v = pb[(m_nOrient + 1) % 4].texCoord.v.v
			= m_texCoord.v.v + hy;
	pb[(m_nOrient + 2) % 4].texCoord.v.v = pb[(m_nOrient + 3) % 4].texCoord.v.v
			= m_texCoord.v.v + m_deltaUV - hy;
