...
for (nSide = 0, sideP = segP->sides; nSide < 6; nSide++, sideP++) {
	...
	if (bInit) {
		...
		nVert = (nFaces [nSide] == 1) ? vlP [0] : std::min(vlP [0], vlP [2]);
...
#endif
	vOffset[X] = f2fl (pParticle->nWidth) / decay;
	vOffset[Y] = f2fl (pParticle->nHeight) / decay;
	}
else {
	vOffset[X] = f2fl (pParticle->nWidth) * decay;
	vOffset[Y] = f2fl (pParticle->nHeight) * decay;
