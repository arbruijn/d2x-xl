...
if (objP->info.nType == OBJ_ROBOT) {
	...
	int32_t f = SEGMENT (objP->info.nSegment)->m_function;
...
if (SHOW_LIGHTNING (1) && gameOpts->render.lightning.bRobots && OBJECT_EXISTS (objP)) {
	...
	if (0 <= m_objects [nObject])
		...
		int32_t h = lightningManager.Create (robotLightningInfo, &objP->Position (), &OBJPOS (OBJECT (LOCALPLAYER.nObject))->vPos, NULL, nObject);
		if (h >= 0)
			m_objects [nObject] = h;
		}
	}
