...
tempv.z = 0;
VmVecAdd (&rod_points [0].p3_vec, &top_point->p3_vec, &tempv);
VmVecSub (&rod_points [1].p3_vec, &top_point->p3_vec, &tempv);
VmVecCopyScale (&tempv, &rod_norm, bot_width);
tempv.z = 0;
VmVecSub (&rod_points [2].p3_vec, &bot_point->p3_vec, &tempv);
VmVecAdd (&rod_points [3].p3_vec, &bot_point->p3_vec, &tempv);
...
if (CalcRodCorners(bot_point,bot_width,top_point,top_width))
	return 0;
return G3DrawPoly(4,rod_point_list);
}
