...
class CBitmap {
	...
	public:
...
#else
		inline int BufSize (void) { return m_info.buffer.Buffer () ? (int) m_info.props.h * (int) m_info.props.rowSize : 0; }
#endif
		...
		inline ubyte* SetBuffer (ubyte *buffer, bool bChild = false) { 
			m_info.buffer.SetBuffer (buffer, bChild); 
			return buffer;
			}
