...
// -- 	if (((robptr->weapon_type2 == -1) || (gun_num != 0)) && (ailp->rapidfire_count < robptr->rapidfire_count[gameStates.app.nDifficultyLevel])) {
...
// -- 	}
	...
	if (((gun_num != 0) || (robptr->weapon_type2 == -1)) && (ailp->rapidfire_count < robptr->rapidfire_count[gameStates.app.nDifficultyLevel])) {
		ailp->next_fire = min(F1_0/8, robptr->firing_wait[gameStates.app.nDifficultyLevel]/2);
	} else {
		if ((robptr->weapon_type2 == -1) || (gun_num != 0)) {
			ailp->next_fire = robptr->firing_wait[gameStates.app.nDifficultyLevel];
...
int lead_player(object *objP, vms_vector *fire_point, vms_vector *believed_player_pos, int gun_num, vms_vector *fire_vec)
{
	...
	VmVecSub(&vec_to_player, believed_player_pos, fire_point);
	...
	fire_vec->z = compute_lead_component(believed_player_pos->z, fire_point->z, gameData.objs.console->mtype.phys_info.velocity.z, projected_time);
...
void ai_fire_laser_at_player(object *objP, vms_vector *fire_point, int gun_num, vms_vector *believed_player_pos)
{
	...
	//this assertion is a problem if a bot with a laser has been modified to also be charging
	...
	if (objP->ctype.ai_info.SUB_FLAGS & SUB_FLAGS_GUNSEG) {
		...
		short	conn_side;
		int	gun_segnum = FindSegByPoint(fire_point, objP->segnum);

		//	See if these segments are connected, which should almost always be the case.
		conn_side = FindConnectedSide(&gameData.segs.segments[gun_segnum], &gameData.segs.segments[objP->segnum]);
		if (conn_side != -1) {
			//	They are connected via conn_side in segment objP->segnum.
			//	See if they are unobstructed.
			if (!(WALL_IS_DOORWAY(gameData.segs.segments + objP->segnum, conn_side, NULL) & WID_FLY_FLAG)) {
	...
	if (gameStates.gameplay.seismic.nMagnitude) {
		...
		temp = F1_0 - abs(gameStates.gameplay.seismic.nMagnitude);
		if (temp < F1_0/2)
			temp = F1_0/2;
	...
	if (d_rand() < 16384) {
		if (lead_player(objP, fire_point, believed_player_pos, gun_num, &fire_vec))		//	Stuff direction to fire at in fire_point.
			goto player_led;
	}
...
player_led: ;
	...
	CreateNewLaserEasy( &fire_vec, fire_point, OBJ_IDX (objP), weapon_type, 1);
...
void ai_do_actual_firing_stuff(object *objP, ai_static *aip, ai_local *ailp, robot_info *robptr, vms_vector *vec_to_player, fix dist_to_player, vms_vector *gun_point, int player_visibility, int object_animates, int gun_num)
{
	fix	dot;
	//robot_info *robptr = gameData.bots.pInfo + objP->id;
	...
	if ((player_visibility == 2) || 
		...
		if (((!object_animates) || (ailp->achieved_state[aip->CURRENT_GUN] == AIS_FIRE))
			...
			if (!ai_multiplayer_awareness(objP, ROBOT_FIRE_AGITATION))
				return;
			ai_fire_laser_at_player(objP, gun_point, gun_num, &gameData.ai.vBelievedPlayerPos);
				...
				if (aip->CURRENT_GUN < robptr->n_guns) {
					if (robptr->attack_type == 1) {
						...
						if ((gun_point->x == 0) && (gun_point->y == 0) && (gun_point->z == 0)) {
							...
							if (gun_num != 0) {
								if (ailp->next_fire <= 0)
									ai_fire_laser_at_player(objP, gun_point, gun_num, &Last_fired_upon_player_pos);

								if ((ailp->next_fire2 <= 0) && (robptr->weapon_type2 != -1)) {
									calc_gun_point(gun_point, objP, 0);
									ai_fire_laser_at_player(objP, gun_point, 0, &Last_fired_upon_player_pos);
								}
