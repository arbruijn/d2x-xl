...
if (!cf.Open ("descent.cfg", gameFolders.szConfigFolder, "rb", 0))
	return 1;
while (!cf.EoF ()) {
	memset (line, 0, 80);
	cf.GetS (line, 80);
...
if (cf.Open ("descentw.cfg", gameFolders.szConfigFolder, "rb", 0)) {
	while (!cf.EoF ()) {
		memset (line, 0, 80);
		cf.GetS (line, 80);
		ptr = line;
...
if (!cf.Open ("descent.cfg", gameFolders.szConfigFolder, "wt", 0))
	return 1;
