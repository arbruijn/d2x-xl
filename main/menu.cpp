...
do {
	...
	if (gameOpts->app.bExpertMode) {
		...
		*szSpeedBoost = *(TXT_SPEEDBOOST - 1);
		m.AddSlider (nOptions, szSpeedBoost + 1, extraGameInfo [0].nSpeedBoost, 0, 10, KEY_B, HTX_GPLAY_SPEEDBOOST);
		...
		*szFusionRamp = *(TXT_FUSION_RAMP - 1);
		m.AddSlider (nOptions, szFusionRamp + 1, extraGameInfo [0].nFusionRamp - 2, 0, 6, KEY_U, HTX_FUSION_RAMP);
		physOpts.nFusionRamp = nOptions++;
		sprintf (szOmegaRamp + 1, TXT_OMEGA_RAMP, OmegaRampStr ());
		*szOmegaRamp = *(TXT_OMEGA_RAMP - 1);
		m.AddSlider (nOptions, szOmegaRamp + 1, extraGameInfo [0].nOmegaRamp, 0, 6, KEY_O, HTX_OMEGA_RAMP);
		...
		*szMslTurnSpeed = *(TXT_MSL_TURNSPEED - 1);
		m.AddSlider (nOptions, szMslTurnSpeed + 1, extraGameInfo [0].nMslTurnSpeed, 0, 2, KEY_T, HTX_GPLAY_MSL_TURNSPEED);
		...
		*szMslStartSpeed = *(TXT_MSL_STARTSPEED - 1);
		m.AddSlider (nOptions, szMslStartSpeed + 1, 3 - extraGameInfo [0].nMslStartSpeed, 0, 3, KEY_S, HTX_MSL_STARTSPEED);
		...
		*szSlowmoSpeedup = *(TXT_SLOWMOTION_SPEEDUP - 1);
		m.AddSlider (nOptions, szSlowmoSpeedup + 1, gameOpts->gameplay.nSlowMotionSpeedup, 0, 4, KEY_M, HTX_SLOWMOTION_SPEEDUP);
		...
		*szDebrisLife = *(TXT_DEBRIS_LIFE - 1);
		m.AddSlider (nOptions, szDebrisLife + 1, gameOpts->render.nDebrisLife, 0, 8, KEY_D, HTX_DEBRIS_LIFE);
		nOptDebrisLife = nOptions++;
		sprintf (szDrag + 1, TXT_PLAYER_DRAG, extraGameInfo [0].nDrag * 10, '%');
		*szDrag = *(TXT_PLAYER_DRAG - 1);
		m.AddSlider (nOptions, szDrag + 1, extraGameInfo [0].nDrag, 0, 10, KEY_G, HTX_PLAYER_DRAG);
		...
		if (!extraGameInfo [0].nDrag)
			...
			m.AddCheck (nOptions, TXT_WIGGLE_SHIP, extraGameInfo [0].bWiggle, KEY_W, HTX_MISC_WIGGLE);
			optWiggle = nOptions++;
			}
		m.AddCheck (nOptions, TXT_BOTS_HIT_BOTS, extraGameInfo [0].bRobotsHitRobots, KEY_O, HTX_GPLAY_BOTSHITBOTS);
		optRobHits = nOptions++;
		m.AddCheck (nOptions, TXT_FLUID_PHYS, extraGameInfo [0].bFluidPhysics, KEY_Y, HTX_GPLAY_FLUIDPHYS);
		optFluidPhysics = nOptions++;
		m.AddCheck (nOptions, TXT_USE_HITANGLES, extraGameInfo [0].bUseHitAngles, KEY_H, HTX_GPLAY_HITANGLES);
		optHitAngles = nOptions++;
		m.AddText (nOptions, "", 0);
		nOptions++;
		m.AddRadio (nOptions, TXT_IMMUNE_MISSILES, 0, KEY_I, 1, HTX_KILL_MISSILES);
		optKillMissiles = nOptions++;
		m.AddRadio (nOptions, TXT_OMEGA_KILLS_MISSILES, 0, KEY_K, 1, HTX_KILL_MISSILES);
		nOptions++;
		m.AddRadio (nOptions, TXT_KILL_MISSILES, 0, KEY_A, 1, HTX_KILL_MISSILES);
		nOptions++;
		m [optKillMissiles + extraGameInfo [0].bKillMissiles].value = 1;
		m.AddText (nOptions, "", 0);
		nOptions++;
		m.AddRadio (nOptions, TXT_AUTOLEVEL_NONE, 0, KEY_N, 2, HTX_AUTO_LEVELLING);
		optAutoLevel = nOptions++;
		m.AddRadio (nOptions, TXT_AUTOLEVEL_SIDE, 0, KEY_S, 2, HTX_AUTO_LEVELLING);
		nOptions++;
		m.AddRadio (nOptions, TXT_AUTOLEVEL_FLOOR, 0, KEY_F, 2, HTX_AUTO_LEVELLING);
		nOptions++;
		m.AddRadio (nOptions, TXT_AUTOLEVEL_GLOBAL, 0, KEY_M, 2, HTX_AUTO_LEVELLING);
		nOptions++;
		m [optAutoLevel + NMCLAMP (gameOpts->gameplay.nAutoLeveling, 0, 3)].value = 1;
		m.AddText (nOptions, "", 0);
		nOptions++;
		m.AddRadio (nOptions, TXT_HIT_SPHERES, 0, KEY_W, 3, HTX_GPLAY_HITBOXES);
		optHitboxes = nOptions++;
		m.AddRadio (nOptions, TXT_SIMPLE_HITBOXES, 0, KEY_W, 3, HTX_GPLAY_HITBOXES);
		nOptions++;
		m.AddRadio (nOptions, TXT_COMPLEX_HITBOXES, 0, KEY_W, 3, HTX_GPLAY_HITBOXES);
		nOptions++;
...
do {
	...
	m.AddSlider (nOptions, TXT_FX_VOLUME, gameConfig.nDigiVolume, 0, 8, KEY_F, HTX_ONLINE_MANUAL);
	soundOpts.nDigiVol = nOptions++;
	m.AddSlider (nOptions, gameStates.sound.bRedbookEnabled ? TXT_CD_VOLUME : TXT_MIDI_VOLUME, 
	...
	if (gameStates.app.bGameRunning || gameStates.app.bNostalgia)
		...
		*szChannels = *(TXT_SOUND_CHANNEL_COUNT - 1);
		m.AddSlider (nOptions, szChannels + 1, gameStates.sound.nSoundChannels, 0, 
	...
	if (!gameStates.app.bNostalgia) {
		...
		*szVolume = *(TXT_CUSTOM_SOUNDVOL - 1);
		m.AddSlider (nOptions, szVolume + 1, gameOpts->sound.xCustomSoundVolume, 0, 
	...
	if (gameStates.app.bNostalgia || !gameOpts->sound.xCustomSoundVolume) 
		...
		m.AddText (nOptions, "", 0);
		nOptions++;
		m.AddCheck (nOptions, TXT_SHIP_SOUND, gameOpts->sound.bShip, KEY_S, HTX_SHIP_SOUND);
		optShipSound = nOptions++;
		m.AddCheck (nOptions, TXT_MISSILE_SOUND, gameOpts->sound.bMissiles, KEY_M, HTX_MISSILE_SOUND);
		optMissileSound = nOptions++;
		m.AddCheck (nOptions, TXT_GATLING_SOUND, gameOpts->sound.bGatling, KEY_G, HTX_GATLING_SOUND);
		soundOpts.nGatling = nOptions++;
		if (gameOpts->sound.bGatling) {
			m.AddCheck (nOptions, TXT_SPINUP_SOUND, extraGameInfo [0].bGatlingSpeedUp, KEY_U, HTX_SPINUP_SOUND);
			optSpeedUpSound = nOptions++;
			}
		}
	m.AddText (nOptions, "", 0);
	nOptions++;
	m.AddCheck (nOptions, TXT_REDBOOK_ENABLED, gameStates.sound.bRedbookPlaying, KEY_C, HTX_ONLINE_MANUAL);
	soundOpts.nRedbook = nOptions++;
	m.AddCheck (nOptions, TXT_REVERSE_STEREO, gameConfig.bReverseChannels, KEY_R, HTX_ONLINE_MANUAL);
	optReverse = nOptions++;
	if (gameStates.sound.bRedbookEnabled || !gameConfig.nMidiVolume)
		optFadeMusic = -1;
	else {
		m.AddCheck (nOptions, TXT_FADE_MUSIC, gameOpts->sound.bFadeMusic, KEY_F, HTX_FADE_MUSIC);
