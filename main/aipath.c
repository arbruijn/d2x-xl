...
void move_towards_outside(point_seg *psegs, int *num_points, object *objP, int rand_flag)
{
	...
	for (i=1; i<*num_points-1; i++) {
		...
		// -- psegs[i].segnum = FindSegByPoint(&psegs[i].point, psegs[i].segnum);
		temp_segnum = FindSegByPoint(&psegs[i].point, psegs[i].segnum);
		Assert(temp_segnum != -1);
		psegs[i].segnum = temp_segnum;
		segnum = psegs[i].segnum;

		VmVecSub(&a, &psegs[i].point, &psegs[i-1].point);
		VmVecSub(&b, &psegs[i+1].point, &psegs[i].point);
		VmVecSub(&c, &psegs[i+1].point, &psegs[i-1].point);
		//	I don't think we can use quick version here and this is _very_ rarely called. --MK, 07/03/95
		VmVecNormalizeQuick(&a);
		VmVecNormalizeQuick(&b);
...
if (VmVecMagQuick(&e) < F1_0/2)
	Int3();

		segment_size = VmVecDistQuick(&gameData.segs.vertices[gameData.segs.segments[segnum].verts[0]], &gameData.segs.vertices[gameData.segs.segments[segnum].verts[6]]);
		if (segment_size > F1_0*40)
			segment_size = F1_0*40;

		VmVecScaleAdd(&goal_pos, &psegs[i].point, &e, segment_size/4);
		...
		new_segnum = FindSegByPoint(&goal_pos, psegs[i].segnum);
...
void create_n_segment_path(object *objP, int path_length, short avoid_seg)
{
	...
	if (gameData.ai.localInfo[OBJ_IDX (objP)].previous_visibility) {
		if (aip->path_length) {
			...
			move_towards_outside(&gameData.ai.pointSegs[aip->hide_index], &t_num_points, objP, 1);
