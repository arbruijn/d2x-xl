...
//of faces
int SpecialCheckLineToFace(vms_vector *newp, vms_vector *p0, vms_vector *p1, segment *seg, int side, int facenum, int nv, fix rad)
{
	vms_vector move_vec;
...
if (disable_new_fvi_stuff)
	return CheckLineToFace(newp, p0, p1, seg, side, facenum, nv, rad);
...
if (edgemask = CheckPointToFace (p0, s, facenum, nv, vertex_list))
	return CheckLineToFace (newp, p0, p1, seg, side, facenum, nv, rad);
for (edgenum = 0; !(edgemask & 1); edgemask >>= 1, edgenum++)
	;
...
hit_seg2 = fviHitData.nSegment2 = -1;
nHitType = fvi_sub (&vHitPoint, &hit_seg2, fq->p0, (short) fq->startSeg, fq->p1, fq->rad, 
...
if (hit_seg2!=-1 && !GetSegMasks(&vHitPoint, hit_seg2, 0).centerMask)
	nHitSegment = hit_seg2;
else
	nHitSegment = FindSegByPoint(&vHitPoint, fq->startSeg);
//MATT: TAKE OUT THIS HACK AND FIX THE BUGS!
if (nHitType == HIT_WALL && nHitSegment==-1)
	if (fviHitData.nSegment2!=-1 && GetSegMasks(&vHitPoint, fviHitData.nSegment2, 0).centerMask==0)
...
if (endMask != 0) {                             //on the back of at least one face
	...
	for (side = 0, bit = 1; (side < 6) && (endMask >= bit); side++) {
		...
		for (face = 0; face < nFaces; face++, bit <<= 1) {
			if (endMask & bit) {            //on the back of this face
				...
				if (nFaceHitType) {            //through this wall/door
					...
					if ((wid_flag & WID_FLY_FLAG) ||
						...
						//do the check recursively on the next segP.
						...
						if (i == nSegsVisited) {                //haven't visited here yet
							...
							if (subHitType != HIT_NONE) {
								...
								if (d < dMin) {
									...
									if (flags&FQ_GET_SEGLIST) {
										int ii;
										for (ii=0;i<nTempSegs && *nSegments<MAX_FVI_SEGS-1;)
											segList [(*nSegments)++] = tempsegList [ii++];
										}
								...
								if (flags&FQ_GET_SEGLIST) {
									...
									for (ii=0;ii<nTempSegs && ii<MAX_FVI_SEGS-1;ii++)
										hitNonesegList [ii] = tempsegList [ii];
									}
		...
		for (side=0, bit=1;side<6 && faceMask>=bit;side++) {
			...
			for (face=0;face<2;face++, bit<<=1) {
				...
				if (faceMask & bit) {            //on the back of this face
					...
					nFaceHitType = check_sphere_to_face(pnt, &seg->sides [side], 
