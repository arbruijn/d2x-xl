...
int check_line_to_face(vms_vector *newp,vms_vector *p0,vms_vector *p1,segment *seg,int side,int facenum,int nv,fix rad)
{
	...
	create_abs_vertex_lists(&num_faces,vertex_list,SEG_IDX (seg),side);
			...
			for (face=0;face<2;face++,bit<<=1) {
				if (endmask & bit) {            //on the back of this face
					...
					if (face_hit_type) {            //through this wall/door
						...
						if ( (thisobjnum == gameData.multi.players[gameData.multi.nLocalPlayer].objnum) && (gameStates.app.cheats.bPhysics==0xBADA55) )	{
						...
						if ((wid_flag & WID_FLY_FLAG) ||
							...
							if (i==n_segs_visited) {                //haven't visited here yet
								...
								sub_hit_type = fvi_sub (&sub_hit_point, &sub_hit_seg, p0, (short) newsegnum,
																p1, rad, thisobjnum, ignore_obj_list, flags,
																temp_seglist, &temp_n_segs, startseg);
