...
#define WI_homing_flag(_i)		(gameStates.app.bD1Mission ? weaponInfoD2D1 [_i].homing_flag : gameData.weapons.info [_i].homing_flag)
#define WI_set_homing_flag(_i,_v)		{ if (gameStates.app.bD1Mission) weaponInfoD2D1 [_i].homing_flag = (_v); else gameData.weapons.info [_i].homing_flag = (_v); }
...
extern void DoSelectWeapon(int weapon_num, int secondary_flag);
...
extern void AutoSelectWeapon(int weapon_type, int auto_select);        //parm is primary or secondary
extern void SelectWeapon(int weapon_num, int secondary_flag, int print_message,int wait_for_rearm);
...
//      HAS_AMMO_FLAG
//      HAS_SUPER_FLAG
extern int PlayerHasWeapon(int weapon_num, int secondary_flag, int nPlayer);

//called when one of these weapons is picked up
//when you pick up a secondary, you always get the weapon & ammo for it
int PickupSecondary (object *objP, int weapon_index, int count, int nPlayer);
...
int PickupAmmo(int class_flag,int weapon_index,int ammo_count, int nPlayer);

extern int AttemptToStealItem(object *objp, int player_num);

//this function is for when the player intentionally drops a powerup
extern int SpitPowerup(object *spitter, ubyte id, int seed);
