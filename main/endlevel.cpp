...
if (exitFlightDataP->firstTime || (UpdateObjectSeg (objP, false) > 0)) {
	...
	if ((nExitSide >= 0) && (segP->m_children [nExitSide] >= 0)) {
		...
		for (int i = 0; i < 6; i++) {
			d = CFixVector::Dot (segP->m_sides [i].m_normals [2], exitFlightDataP->objP->info.position.mOrient.m.dir.u);
			if (d > dLargest) {
				dLargest = d; 
				nUpSide = i;
				}
