...
extern int gameStates.multi.bServer;
...
static int UDPOpenSocket(ipx_socket_t *sk, int port) {
	...
	if (setsockopt(sk->fd, SOL_SOCKET, SO_BROADCAST, (char *)&val_one, sizeof(val_one))) {
		if (closesocket(sk->fd)) 
			msg("closesocket() failed during error recovery: %m");
		sk->fd=-1;
...
//		/*gameStates.multi.bServer?htonl(INADDR_ANY):*/htonl(*((u_long *) (ipx_MyAddress+4)));
	sin.sin_port=htons(baseport);
//	if ((gameStates.multi.bServer?bind:connect)(sk->fd,(struct sockaddr *)&sin,sizeof(sin))) {
