...
nDepth = THIEF_DEPTH;
while (nSegment == -1) {
	nSegment = PickConnectedSegment (gameData.Object (LOCALPLAYER.nObject), nDepth, &nDropDepth);
	if (nDropDepth < THIEF_DEPTH / 2)
		return (RandShort () * gameData.segs.nLastSegment) >> 15;
	if ((nSegment >= 0) && (gameData.Segment (nSegment)->m_function == SEGMENT_FUNC_REACTOR))
		nSegment = -1;
	nDepth--;
	}
...
newObjP = CreateMorphRobot( gameData.Segment (nSegment), &vCenter, objP->info.nId);
...
int32_t AttemptToStealItem3(CObject *objP, int32_t nPlayer)
{
	int32_t i;
...
int32_t AttemptToStealItem2(CObject *objP, int32_t nPlayer)
{
int32_t rval = AttemptToStealItem3 (objP, nPlayer);
if (rval) {
...
//	If a wapon successfully stolen, do everything, removing it from player,
//	updating gameData.thief.stolenItems information, deselecting, etc.
int32_t AttemptToStealItem (CObject *objP, int32_t nPlayer)
{
	int32_t	i;
	int32_t	rval = 0;
