...
vPerturb += parentObjP->info.position.mOrient.m.v.u * (-I2X (1) / 2);
for (i = 0; i < nOmegaBlobs; i++) {
	CFixVector	vTempPos;
...
if (nTargetObj != -1)
	...
	vPerturb = CFixVector::Random();
	perturbed_fvec = bSpectate ? gameStates.app.playerPos.mOrient.m.v.f : parentObjP->info.position.mOrient.m.v.f
	               + vPerturb * (I2X (1) / 16);
