...
if (objP->info.nType == OBJ_PLAYER) {
	...
	vDefaultGunPoints = ROBOTINFO (objP) ? ROBOTINFO (objP)->gunPoints : 0;
...
if (gameStates.app.cheats.bHomingWeapons || gameData.weapons.info [nLaserType].homingFlag) {
	if (objP == gameData.objData.consoleP) {
		weaponP->cType.laserInfo.nHomingTarget = weaponP->FindVisibleHomingTarget (vLaserPos, MAX_THREADS);
...
if ((objP = GuidedInMainView ())) {
	nObject = objP->FindVisibleHomingTarget (objP->info.position.vPos, MAX_THREADS);
	gameData.objData.trackGoals [0] =
...
for (i = 0; i < j; i++, h = !h) {
	...
	if ((vGunPoints = GetGunPoints (gameData.objData.consoleP, nGun))) {
		...
		nObject = gameData.objData.consoleP->FindVisibleHomingTarget (vGunPos, MAX_THREADS);
